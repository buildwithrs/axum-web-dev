...
mod tests {
    ...
    async fn test_get_user() {
        ...
        let expected = format!("user_id: {}", 1001);
    ...
    async fn test_create_user_happy_path() {
        ...
        let body_bytes = to_bytes(resp.into_body(), 1024).await.unwrap();
        let expected = json!(CreateUserResp {
            id: "1001x".to_string(),
            name: "Alex".to_string()
        })
        .to_string();
    ...
    #[tokio::test]
    async fn test_create_user_invalid_args() {
        let router = app_router();
        let req = Request::builder()
            .method(http::Method::POST)
            .uri("/api/u/users")
        ...
        let body_bytes = to_bytes(resp.into_body(), 1024).await.unwrap();
        let expected = format!("{}", "user id empty or too short");
        assert_eq!(body_bytes, expected);
    }
}
